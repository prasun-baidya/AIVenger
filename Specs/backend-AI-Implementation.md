...
## Table of Contents
- [Quick Changes Overview](#quick-changes-overview)
- [Backend Integration: Avatar Generation System](#backend-integration-avatar-generation-system)
...
- [AI Integration: Gemini 2.5 Flash Image](#ai-integration-gemini-25-flash-image)
...
### Optional: Deprecate localStorage Helper
...
export interface GenerateAvatarError {
  success: false;
  error: string;
  code: "INSUFFICIENT_CREDITS" | "UPLOAD_FAILED" | "AI_GENERATION_FAILED" | "DATABASE_ERROR" | "UNAUTHORIZED";
}
...
- **Three-stage upload process** - Original → AI generation → Re-upload generated (ensures permanent storage)
...
## AI Integration: Gemini 2.5 Flash Image
...
**Documentation:**
- [OpenRouter Model Page](https://openrouter.ai/models)
- [Image Generation Guide](https://openrouter.ai/docs/image-generation)
- [Prompting Best Practices](https://openrouter.ai/docs/prompting)
...
- Add new sections after "Next Steps After Implementation":
  - Backend Integration: Avatar Generation System (detailed database schema, API routes, server actions)
...
### To Create
5. `docs/technical/ai/avatar-generation-implementation.md` - Comprehensive implementation documentation with research and detailed code examples
...
1. **Credits Deducted Before AI Call** - Prevents abuse (users can't retry infinitely on failures). This is intentional and won't refund on AI errors.
2. **Three-Stage Upload** - Original image uploaded first, then AI-generated image downloaded and re-uploaded. Ensures permanent storage in blob.
3. **No Real-Time Progress** - Initial implementation uses simple loading state. Future enhancement: WebSocket or polling for progress updates.
4. **Fixed Prompt** - Superhero transformation prompt is hardcoded. Future enhancement: Let users choose styles (Marvel, DC, anime, etc.).
